...
- **Indices**: S&P 500 (**^GSPC**, i.e. SPX — the literal index level, not the SPY ETF), Russell 2000, KOSPI, and SSE Composite (**^RUT**, **^KS11**, **000001.SS**) all show the raw index value rather than a `$` price. Nasdaq and Dow Jones are still tracked via a liquid ETF proxy — **QQQ** and **DIA** respectively (labeled "via QQQ" etc.) — since there's no cleaner single-ticker way to track those live on Yahoo's public endpoint. Sourced from Yahoo Finance's public chart endpoint (6 months of daily closes per index), cached in KV for **15 minutes** — short enough that the pre/post-market read below (see next bullet) doesn't go stale for long between visits, while `force: true` (the Refresh button) always bypasses this and hits Yahoo live regardless.
