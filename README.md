...
**Bring your own Gemini key**: the gear icon (top right, every tab) opens Settings, where anyone can paste their own free (or paid) Gemini API key from [aistudio.google.com/apikey](https://aistudio.google.com/apikey). It's saved in that browser's `localStorage` only — never sent anywhere but this Worker, and never logged or stored server-side. When present, it's sent with the `/analyze` request and the Worker uses it instead of the shared `GEMINI_API_KEY`, so that person's Analyze calls draw on their own quota/plan instead of the pool everyone else shares. No key set → falls back to the shared key, same as before. This is meant to become mandatory once the app moves past a small testing group — the shared key exists for now purely so nobody's blocked while trying it out.
