...
- **Indices**: S&P 500 (**^GSPC**, i.e. SPX), Nasdaq (**^IXIC**, the Nasdaq Composite), Russell 2000, KOSPI, and SSE Composite (**^RUT**, **^KS11**, **000001.SS**) all show the literal index level rather than a `$` ETF price. Dow Jones is still tracked via a liquid ETF proxy — **DIA** (labeled "via DIA") — since there's no single-ticker way to pull the Dow itself from Yahoo's public endpoint. Sourced from Yahoo Finance's public chart endpoint (6 months of daily closes per index), cached in KV for **15 minutes** — short enough that the pre/post-market read below (see next bullet) doesn't go stale for long between visits, while `force: true` (the Refresh button) always bypasses this and hits Yahoo live regardless.
